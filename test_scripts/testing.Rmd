...
final_data <- calculate_label_rate(svm_data_filtered, quiet = FALSE) %>% 
  #filter data to remove bad fits, curves with insuffucuent data
  filter(enough_data, !fit_error) %>% 
  #remove columns with the error information
...
test <- function(variable, bins=100) {
  var_quo <- enquo(variable)
  bind_rows(
    svm_data_rates %>% mutate(ds="set1", label_rate = label_rate+0.1),
    svm_data_rates %>% mutate(ds="set2")
  ) %>% 
    ggplot() + 
    aes_q(x = var_quo, y = quo(..density..), 
          fill = quo(ds), color = quo(ds)) + 
    #geom_vline(xintercept = 0.066)+
    geom_histogram(
      bins = bins, color = "black", alpha = 0.5) +
    geom_density(map = aes(fill = NULL), size = 2)+
    labs(y = "probability [%]") + 
    scale_y_continuous(expand = c(0,0)) +
    theme_bw()
}

test(label_rate, bins = 10)
```
